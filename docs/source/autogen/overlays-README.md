...
For instance I override the version of [`starship`](https://github.com/HugoHakem/nix-os.config/blob/main/overlays/master-pkgs.nix) by using the `mpkgs` which is the `nixpkgs` following the `master branch`. This is useful if you need the latest available package for some packages without having to update the `nixpkgs`channel risking to breaks the whole configuration.
