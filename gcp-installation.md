...
Now that the Nix package manager is available, pull the configuration file from this repo. Nix makes this convenient and allows you to run flakes hosted on git repositories. Here, you will run the `#install` command specified in [`apps/x86_64-linux/install`](./apps/x86_64-linux/install) as a shell script. It is made available as a Nix app with the `mkApp` function in [`flake.nix`](./flake.nix).
...
2. Run the apply function (which, if you are curious, is a bash script detailed in [`apps/x86_64-linux/apply`](./apps/x86_64-linux/apply)):

    ```bash
    nix run .#apply
    ```

This will override the following lines in [flake.nix](./flake.nix):
...
Again, if you are curious, the `build-switch` app is defined in [apps/x86_64-linux/build-switch](./apps/x86_64-linux/build-switch).
...
- If you believe the package you want to install is Linux-specific, note that this configuration is intended to be compatible with both macOS and Linux. In such cases, you should modify the [modules/linux/](./modules/linux/README.md) configuration.
...
This is what [modules/shared/config/gcp-ssh-script.sh](./modules/shared/config/gcp-ssh-script.sh) does. It is supposedly the default, but you may have to make it executable. Run in your home directory:
