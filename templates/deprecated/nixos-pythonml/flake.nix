      ...
      let
        ...
        LD_pkgs = # PACKAGES FOR LD_LIBRARY_PATH
          (with pkgs; [
            ...
            (with pkgs; [
              # ADD NEEDED PACKAGES HERE IF ONLY REQUIRED BY LINUX
              # This is required for most app that uses graphics api
              # linuxPackages.nvidia_x11
            ]) ++ 
            (with mpkgs.cudaPackages; [ # GPU UTILITIES
                libcublas
                libcurand
                cudnn
                libcufft
        ...
        devShells = {
          default =
            ...
            mkShell {
              ...
              packages = [python_with_pkgs] ++ shell_pkgs ++ LD_pkgs;
              venvDir = "./.venv";
              postVenvCreation = ''
              ...
              shellHook = ''
                ...
                export CUDA_PATH=${mpkgs.cudaPackages.cudatoolkit}
