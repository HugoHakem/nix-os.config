...
This templates provide an example of how to set up a **Machine Learning project**.
...
+ You will specify your python version with `nix`.
+ If you have packages that is not working when installing from `uv`, you may want to specify it with `nix` (e.g. `pytorch`, might not recognize your GPU, and specifying with `nix` might solve the issue. However its version will be immutable and won't be able to change unless you do it through `nix` - Hence, the reason why `uv` is more preferred to handle python dependencies.
